...
vector<Coordinate> GetValidUnvisitedNeighbors(const vector<vector<Color>>& maze, const Coordinate& c) {
	vector<Coordinate> ret;
	if (c.x - 1 >= 0 && maze[c.x - 1][c.y] != kBlack)
		ret.emplace_back(Coordinate{ c.x - 1, c.y });
	if (c.x + 1 < maze.size() && maze[c.x + 1][c.y] != kBlack)
		ret.emplace_back(Coordinate{ c.x + 1, c.y });
	if (c.y - 1 >= 0 && maze[c.x][c.y - 1] != kBlack)
		ret.emplace_back(Coordinate{ c.x, c.y - 1 });
	if (c.y + 1 < maze[c.x].size() && maze[c.x][c.y + 1] != kBlack)
		ret.emplace_back(Coordinate{ c.x, c.y + 1 });

	return ret;
}

bool SearchMazeHelper(vector<vector<Color>>& maze, vector<Coordinate>& path, const Coordinate& c, const Coordinate& e) {
	path.emplace_back(c);
	...
	maze[c.x][c.y] = kBlack;
	auto neighbors = GetValidUnvisitedNeighbors(maze, c);
	for (auto& neighbor : neighbors) {
		if (SearchMazeHelper(maze, path, neighbor, e))
			return true;
	}
